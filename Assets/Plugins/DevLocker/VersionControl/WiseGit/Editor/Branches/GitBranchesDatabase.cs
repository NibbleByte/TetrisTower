...
namespace DevLocker.VersionControl.WiseGit.Branches
{
	...
	/// </summary>
	public class GitBranchesDatabase : Utils.DatabasePersistentSingleton<GitBranchesDatabase, BranchProject>
	{
		...
		private void GatherProjectsIn(BranchScanParameters scanParams, List<BranchProject> results)
		{
			...
			while (urls.Count > 0) {
				...
				listEntries.Clear();
				var opResult = WiseGitIntegration.ListURL(url, false, listEntries);
