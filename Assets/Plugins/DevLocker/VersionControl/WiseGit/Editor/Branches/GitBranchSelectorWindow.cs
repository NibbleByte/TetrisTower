...
namespace DevLocker.VersionControl.WiseGit.Branches
{
	...
	/// </summary>
	public class GitBranchSelectorWindow : EditorWindow
	{
		...
		private GitBranchesDatabase Database => GitBranchesDatabase.Instance;
		...
		[MenuItem("Assets/Git/Branch Selector", false, GitContextMenusManager.MenuItemPriorityStart + 140)]
		private static void OpenBranchesSelector()
		{
			var window = CreateInstance<GitBranchSelectorWindow>();
		...
		private void InitializeStyles()
		{
			...
			RepoBrowserContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-RepoBrowser", "Repo-Browser in this branch at the target asset.");
			ShowLogContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ShowLog", showLogTooltip);
			SwitchBranchContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-Switch", "Switch working copy to another branch.\nOpens TortoiseSVN dialog.");

			ScanForConflictsContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ScanForConflicts", "Scan all branches for potential conflicts.\nThis will look for any changes made to the target asset in the branches.");

			ConflictsPendingContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ConflictsScan-Pending", "Pending - waiting to be scanned for conflicts.\n\n" + showLogTooltip);
			ConflictsFoundContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-Conflicts-Found", "The target asset was modified in this branch - potential conflicts.\n\n" + showLogTooltip);
			ConflictsNormalContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ConflictsScan-Normal", "No conflicts were found by the scan.\n\n" + showLogTooltip);
			ConflictsAddedContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ConflictsScan-Added", "Asset was added in this branch.\n\n" + showLogTooltip);
			ConflictsMissingContent = Preferences.GitPreferencesManager.LoadTexture("Editor/BranchesIcons/SVN-ConflictsScan-Missing", "Asset was missing. It may have been deleted or never existed in this branch.\n\n" + showLogTooltip);
			...
			Preferences.GitPreferencesWindow.MigrateButtonStyleToUIElementsIfNeeded(MiniIconButtonlessStyle);
			Preferences.GitPreferencesWindow.MigrateBorderStyleToUIElementsIfNeeded(BorderStyle);
			...
			using (new EditorGUILayout.VerticalScope()) {
				if (!Database.IsActive) {
					...
					if (GUILayout.Button("Open Project Preferences")) {
						Preferences.GitPreferencesWindow.ShowProjectPreferences(Preferences.GitPreferencesWindow.PreferencesTab.Project);
					}
			...
			using (var scrollView = new EditorGUILayout.ScrollViewScope(m_BranchesScroll)) {
				...
				foreach (var branchProject in Database.BranchProjects) {
					...
					using (new EditorGUILayout.HorizontalScope(/*BranchRowStyle*/)) {
						...
						if (repoBrowser) {
							GitContextMenusManager.RepoBrowser(branchProject.UnityProjectURL + "/" + AssetDatabase.GetAssetPath(m_TargetAsset));
						}

						if (showLog) {
							GitContextMenusManager.ShowLog(branchProject.UnityProjectURL + "/" + AssetDatabase.GetAssetPath(m_TargetAsset));
						}
						...
						if (switchBranch) {
							...
							if (confirm && UnityEditor.SceneManagement.EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()) {
								var localPathNative = WiseGitIntegration.WorkingCopyRootPath();
								...
								if (branchProject.BranchURL != branchProject.UnityProjectURL) {
									...
									if (!useBranchRoot) {
										localPathNative = WiseGitIntegration.ProjectRootNative;
										targetUrl = branchProject.UnityProjectURL;
									}
								}

								GitContextMenusManager.Switch(localPathNative, targetUrl);
		...
		private static void GatherConflicts(object param)
		{
			...
			for (int i = 0; i < results.Length; ++i) {
				...
				var targetRelativeURL = WiseGitIntegration.AssetPathToRelativeURL(targetURL);
				...
				if (jobData.LimitType == ConflictsScanLimitType.Revisions) {
					var lastChangedRevision = WiseGitIntegration.LastChangedRevision(targetURL);
				...
				var opResult = WiseGitIntegration.Log(targetURL, logParams, logEntries, 60000 * 5);
